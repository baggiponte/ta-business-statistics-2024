...
lagged_plots <- function(data, name = "data", lags = c(1, 2, 3, 4)) {
  lag_plot <- function(data, name, lag) {
    plot(x = data, y = lag(data, k = lag), main = paste(name, "vs Lag", lag), xlab = name, ylab = paste("Lag", lag))
  }
...
```{r}
nh_temperature_arima$residuals |> qqnorm(); qqline(nh_temperature_arima$residuals)
```
...
check_normality <- function(arima) {
  ...
  residuals |> qqnorm(); qqline(residuals)
